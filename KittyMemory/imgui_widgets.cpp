...
bool ImGui::tab(const char* label, bool selected)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    ImVec2 pos = window->DC.CursorPos;
    ImVec2 size = CalcItemSize(ImVec2(70, 40), 50 + style.FramePadding.x, label_size.y + style.FramePadding.y);
    const ImRect bb(pos, pos + size);
    ItemSize(size, style.FramePadding.y);
    if (!ItemAdd(bb, id))
        return false;
    ...
    draw->AddRectFilled(ImVec2(bb.Min.x + 5, bb.Max.y - 2), bb.Max, ImGui::GetColorU32(ImLerp(ImVec4(0,0,0,0), ImVec4(accentcolor),t)));
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    auto checkedTopColor = GetColorU32(ImVec4(accentcolor));
    auto checkedBottomColor = GetColorU32(ImVec4(accentcolor)) - ImColor(0, 0, 0, 120);
    ...
    if (label_size.x > 0.0f) {
        ...
        //PopStyleColor();
        RenderText(text_bb.Min + ImVec2(0.f, -3.f), label);
    }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    auto TopColor = GetColorU32(ImVec4(accentcolor));
    auto BottomColor = GetColorU32(ImVec4(accentcolor)) - ImColor(0, 0, 0, 120);


    window->DrawList->AddRectFilled(linestart - linesize, backgroundend + linesize, ImColor(60,60,60), style.GrabRounding);
    window->DrawList->AddRectFilledMultiColor(linestart - linesize, lineend + linesize, TopColor,TopColor,BottomColor,BottomColor);
    window->DrawList->AddRect(linestart-linesize, lineend+linesize,ImColor(10,10,10), 0,false,1);
    window->DrawList->AddCircleFilled(lineend, 1 * (anim1[p_data] / 2.0f + 1.0f), GetColorU32(ImVec4(accentcolor)));
    ...
    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x, frame_bb.Min.y + 4),label);//style.FramePadding.y), label);
